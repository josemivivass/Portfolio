// Mapeo ISO 3166-1 alpha-2 ↔ numérico (el TopoJSON usa numérico como feature.id).
// Permite cruzar el código del backend (geoip-lite, alpha-2) con el del mapa.
// Incluye helper countryName() para resolver el nombre del país en es/en con Intl.DisplayNames.
...
export function countryName(alpha2: string | null | undefined, lang: string = 'es'): string {
  if (!alpha2 || alpha2.length !== 2) return '';
  try {
    return getDisplayNames(lang).of(alpha2.toUpperCase()) || alpha2;
  } catch {
    return alpha2;
  }
}
